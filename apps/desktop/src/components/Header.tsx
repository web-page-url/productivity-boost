...
export default function Header({
  ...
  const isSkills = (view as string) === 'skills'
  const [profile, setProfile] = useState<Profile>({ name: '', imageBase64: null })
  const [showProfile, setShowProfile] = useState(false)

  useEffect(() => {
    window.profileAPI.get().then((p) => setProfile(p)).catch(() => {})
  }, [])
      ...
      <header className="flex-shrink-0 border-b border-zinc-900 bg-[#0a0a0c]">
        <div className="flex items-center gap-2.5 px-4 h-12">
          ...
          <button
            ...
            title="Edit profile"
          >
            <div className="h-7 w-7 rounded-lg overflow-hidden flex-shrink-0 border border-zinc-800 group-hover:border-zinc-600 transition-colors">
              {profile.imageBase64 ? (
            ...
            {profile.name && (
              <span className="text-xs text-zinc-400 group-hover:text-zinc-200 transition-colors max-w-[96px] truncate">
                {profile.name}
              </span>
            )}
          </button>
